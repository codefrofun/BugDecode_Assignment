...
class Program
{
    ...
    /* Switch the start time and game time so that the game starts at 0, not a value, and amke sure the gametime is a float */
    ...
    /* Changed float to int and changed 0.55 float to 1. system doesn't compute Y movement with decimals.  */
    const int Y_MOVE_AMOUNT = 1; // Amount to move down
    ...
    static void Main()
    {
        ...
        while (!gameOver)
        {
            ...
            /* made player xp be caclulated by mod 5*/
            Console.Write($"Score: {score} | Xp: {player.Exp % 5} | Lvl: {player.GetLevel()} | Time: {(gametime - startingTime):F2} ");
    ...
    static void MoveEnemies()
    {
        ...
        if (enemyMoveCounter + MOVE_FREQUENCY >= MOVE_FREQUENCY - 1)
        {
            foreach (var enemy in enemies)
            {
                ...
                {
                    /* added if statement, when enemies hit the bottom it's game over. The console clears, the score/timer/xp displays frozen, and the "Game over" message displays.*/
        ...
        for (int j = enemies.Count - 1; j >= 0; j--)
        {
            ...
            for (int i = bullets.Count - 1; i >= 0; i--)
            {
                if (bullets[i].X == enemies[j].X && bullets[i].Y == enemies[j].Y)
                {
                    ...
                    /* added 10 to player damage so that enemy dies in 3 hits */
                    float damage = 5 + 5 + 14 * player.GetLevel() - enemies[j].Defense * 2;
                    ...
                    if (enemies[j].Health <= 0)
                    {
                        ...
                        /* Now the score counter cannot surpass 255 when it is overflowing (15 enemies x 17 points per kill = 255, which will not surpass overflow. 
                         * Now score goes up slower and there is no chance for the game to end with overflow.) */
                        score += (byte)random.Next(10, 17);
                    }
                    break;
                }
            }
...
//PLAYER LEVEL NEEDS TO START AT 1
class Player
{
    ...
    /* made player xp 5 to give player 1 level*/
    public Player(int x, int y) { X = x; Y = y; Exp = 5; XVelocity = 0; keyPressed = false; }
}
